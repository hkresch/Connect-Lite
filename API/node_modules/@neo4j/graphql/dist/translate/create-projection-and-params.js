...
function createProjectionAndParams({ resolveTree, node, context, chainStr, varName, literalElements, resolveType, inRelationshipProjection, isRootConnectionField, }) {
    function reducer(res, field) {
        ...
        if (cypherField) {
            ...
            const expectMultipleValues = (referenceNode || referenceUnion) && cypherField.typeMeta.array ? "true" : "false";
            ...
            const apocStr = `${!cypherField.isScalar && !cypherField.isEnum ? `${param} IN` : ""} apoc.cypher.runFirstColumn("${cypherField.statement}", ${apocParamsStr}, ${expectMultipleValues})${apocWhere ? ` ${apocWhere}` : ""}${unionWhere ? ` ${unionWhere} ` : ""}${!isProjectionStrEmpty ? ` | ${!referenceUnion ? param : ""} ${projectionStr}` : ""}`;
        ...
        if (relationField) {
            ...
            const { inStr, outStr } = (0, get_relationship_direction_1.getRelationshipDirection)(relationField, field.args);
            if (relationField.interface) {
                ...
                let offsetLimitStr = "";
                if (optionsInput) {
                    offsetLimitStr = (0, pagination_1.createOffsetLimitStr)({
                        offset: optionsInput.offset,
                        limit: optionsInput.limit,
                    });
                    if (optionsInput.sort) {
                        const sorts = optionsInput.sort.reduce(sortReducer, []);
                        res.projection.push(`${field.alias}: apoc.coll.sortMulti(${field.alias}, [${sorts.join(", ")}])${offsetLimitStr}`);
                        return res;
                    }
                }
            ...
            let nestedQuery;
            if (optionsInput) {
                const offsetLimit = (0, pagination_1.createOffsetLimitStr)({ offset: optionsInput.offset, limit: optionsInput.limit });
                if (optionsInput.sort) {
                    const sorts = optionsInput.sort.reduce(sortReducer, []);
                    nestedQuery = `${alias}: apoc.coll.sortMulti([ ${innerStr} ], [${sorts.join(", ")}])${offsetLimit}`;
                }
                else {
                    nestedQuery = `${alias}: ${!isArray ? "head(" : ""}[ ${innerStr} ]${offsetLimit}${!isArray ? ")" : ""}`;
                }
            }
            else {
        ...
        if (connectionField) {
            ...
            res.projection.push(`${field.name}: apoc.cypher.runFirstColumn("${connection[0].replace(/("|')/g, "\\$1")} RETURN ${field.name}", { ${runFirstColumnParams.join(", ")} }, false)`);
