...
function translateDelete({ context, node }) {
    ...
    const topLevelMatch = (0, translate_top_level_match_1.default)({ node, context, varName, operation: "DELETE" });
    matchAndWhereStr = topLevelMatch[0];
    cypherParams = { ...cypherParams, ...topLevelMatch[1] };
    const allowAuth = (0, create_auth_and_params_1.default)({
        operations: "DELETE",
        entity: node,
        context,
        allow: {
...
function getDeleteReturn(context) {
    return context.subscriptionsEnabled
        ? [
            `WITH ${constants_1.META_CYPHER_VARIABLE}`,
            `UNWIND ${constants_1.META_CYPHER_VARIABLE} AS m`,
            `RETURN collect(DISTINCT m) AS ${constants_1.META_CYPHER_VARIABLE}`,
        ]
        : [];
}
